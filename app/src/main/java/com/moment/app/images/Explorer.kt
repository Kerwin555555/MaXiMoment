...
import com.moment.app.images.media.MediaExplorerActivity
import com.moment.app.images.media.MediaPickFragment
import com.moment.app.images.media.MediaPreviewActivity
...
object Explorer {
    ...
    interface DataTrackListener {
        ...
        fun onData(method: String, data: List<MediaDirectory>)

        fun onSelect(file: MediaFile, selected: Boolean)
    }
    ...
    interface FileFilter{
         ...
         */
        fun maxPickCountTrigger(file: MediaFile): Boolean

        /**
         *  file.duration / 1000L > maxVideoDuration
         */
        fun maxVideoDurationTrigger(file: MediaFile): Boolean

        /**
         *  file.size.toDouble() / 1048576.0 > maxVideoSize
         */
        fun maxVideoSizeTrigger(file: MediaFile): Boolean

        /**
         *  file.size.toDouble() / 1048576.0 > maxGifSize
         */
        fun maxPreviewGifSizeTrigger(file: MediaFile): Boolean
    ...
    open class DefaultImageFilter : FileFilter{
        ...
        override fun maxPickCountTrigger(file: MediaFile): Boolean {
            return MediaStoreHelper.selectedFiles.size == pickCount && !MediaStoreHelper.selectedFiles.contains(file.path)
        }

        override fun maxVideoDurationTrigger(file: MediaFile): Boolean {
            return file.duration / 1000 > maxVideoDuration
        }

        override fun maxVideoSizeTrigger(file: MediaFile): Boolean {
            try {
...
//                val size = df.format(file.size * 1.0 / (1024 * 1024)).toDouble()
        ...
        override fun maxPreviewGifSizeTrigger(file: MediaFile): Boolean {
            try {
                val size = file.size * 1.0 / (1024 * 1024)
...
//            }
        ...
        fun createFragment(): MediaPickFragment{
            installBundle()
            return MediaPickFragment.newInstance(bundle)
        }

        fun start(requestCode: Int, context: Fragment?) {
            installBundle()
            context?.let {ctx ->
                val intent = Intent(ctx.activity, MediaExplorerActivity::class.java)
        ...
        fun start(requestCode: Int, context: Activity?) {
            installBundle()
            context?.let {ctx ->
                val intent = Intent(ctx, MediaExplorerActivity::class.java)
                intent.putExtras(bundle)
                ctx.startActivityForResult(intent, requestCode)
            }
        }
    ...
    fun mediaPreview(context: AppCompatActivity?, preview: Boolean, index: Int = 0, dir_id: String,bundle: Bundle?) {
        context?.let {
            val intent = Intent(it, MediaPreviewActivity::class.java)
