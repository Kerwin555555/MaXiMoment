...
object CursorParseHelper {
    ...
    @SuppressLint("Range")
    fun parseImagesFromCursor(context:Context, data: Cursor): MutableList<MediaDirectory> {
        val directories: MutableList<MediaDirectory> = mutableListOf()

        val map: LinkedHashMap<String, MediaDirectory> = linkedMapOf()
        ...
        try{
            while (data.moveToNext()) {
                ...
                val filter = Explorer.getRequest()?.getFileFilter()?.filter(path, mimeType, size, width, height) ?: false
                ...
                if (!isExits || filter){
                    ...
                    if (map[bucketId] == null) {
                        val imageDirectory = MediaDirectory()
        ...
        Explorer.getDataTrack()?.onData("parseImagesFromCursor", directories)
        return directories
    }

    fun parseVideosFromCursor(context: Context, data: Cursor): MediaDirectory {
        val videoDir = MediaDirectory()
        ...
        try {
            ...
            e.printStackTrace()
            Explorer.getDataTrack()?.onError("parseVideosFromCursor",e)
        }

        Explorer.getDataTrack()?.onData("parseVideosFromCursor", mutableListOf(videoDir))
        return videoDir
    }
