...
object OkDisplayCompat {

    fun loadThumb(targetView: ImageView, file: MediaFile, sizeMultiplier: Float, dirtyFileAction: ((file:MediaFile) ->Unit) ?= null , placeHolder: Drawable = ColorDrawable(0xFFEEEEEE.toInt())) {
        if (Build.VERSION.SDK_INT >= 29) {
            targetView.setImageDrawable(placeHolder)
            OKThumbLoader.fetchImageThumb(targetView.context, file, { bitmap ->
    ...
    fun load(targetView: ImageView, file: MediaFile, sizeMultiplier: Float = 0.3f, dirtyFileAction: ((file:MediaFile) ->Unit) ?= null,  placeHolder: Drawable = ColorDrawable(0xFF000000.toInt())) {
        targetView.setImageDrawable(placeHolder)
        val context = targetView.context?:return
        if (context is Activity && context.isDestroyed) return
        val defaultOptions =  if (file.isGif) {
            val filter = Explorer.getRequest()?.getFileFilter()
