 ...
 */
class AlbumSearchActivity : AppCompatActivity(), IAlbumInterface.IAlbumDataView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.explorerRecycler.layoutManager = androidx.recyclerview.widget.GridLayoutManager(this, intent?.extras?.getInt(AlbumSearcher.EXTRA_SPAN_COUNT)?:AlbumSearcher.DEFAULT_SPAN_COUNT)
       ...
       binding.explorerBottomBtnPreview.setOnClickListener {
            if (MediaStoreHelper.fetchSelectedFileList().filter { it.isVideo }.toList().isNotEmpty()){
                AlbumSearcher.videoPreview(this,MediaStoreHelper.selectedFiles[0])
            }else {
                AlbumSearcher.mediaPreview(this, true, 0, "", intent.extras)
            }
        }
...
//            Log.d("@@@==>", "explorer_bottom_btn_apply clicked...")
        ...
        Util.setTopPadding(binding.explorerToolbar, this.applicationContext)
        handleIntent()
    }
    ...
    private fun setVideoResultToPrevious(file: AlbumItemFile) {
        val intent = Intent()
        val selectedUris = ArrayList<Uri>()
