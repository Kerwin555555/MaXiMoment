...
import com.moment.app.localimages.album.adapter.AlbumExaminer
import com.moment.app.localimages.utils.Util

class AlbumSkimActivity : AppCompatActivity(), IAlbumInterface.IAlbumDataView {
    ...
    private val loader by lazy {
        IAlbumInterface.MediaLoader(this,intent.extras,this)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (AlbumSearcher.getRequest()?.getFileFilter()?.isSingleSelectMode() == true) {
            ...
            binding.explorerTvSelect.setOnClickListener {
                ...
                AlbumExaminer.check(this@AlbumSkimActivity, file){
    ...
    private fun updateApplyStatus() {
        when {
            AlbumSearcher.getRequest()?.getFileFilter()?.isSingleSelectMode() == true -> {
    ...
    private fun handleIntent() {
        isPreviewMode = intent.getBooleanExtra(AlbumSearcher.EXTRA_PREVIEW_MODE, false)
        clickedIndex = intent.getIntExtra(AlbumSearcher.EXTRA_PREVIEW_INDEX, 0)
    }
    ...
    private fun handleData(){
        try {
            if (isPreviewMode) { //预览模式
                adapter.setData(
                    MediaStoreHelper.fetchSelectedFileList()
                        .filter { !it.isVideo } as MutableList<AlbumItemFile>,true)
            } else {
                latestDirId = intent.getStringExtra(AlbumSearcher.EXTRA_PREVIEW_LATEST_ID) ?: MediaStoreHelper.DIR_ID_ALL
                ...
                MediaStoreHelper.fetchFilesByDirId(latestDirId) { files, _ ->
                    ...
                    //暂时过滤掉视频，获取realIndex
                    val fileList = files.filter { !it.isVideo } as MutableList<AlbumItemFile>
            ...
            AlbumSearcher.getDataTrack()?.onError("MediaPreviewActivity#handleData()", e)
        }
    ...
    private fun setSelectedStatus(file: AlbumItemFile) {
        ...
        Util.setTopPadding(binding.toolBar, this.applicationContext)
        updateApplyStatus()
    }
    ...
    inner class MediaPreviewAdapter(private val context: Context) :
        ...
        fun getItem(position: Int): AlbumItemFile? {
            try {
        ...
        fun setData(data: MutableList<AlbumItemFile>, forceClear: Boolean = false) {
